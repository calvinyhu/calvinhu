...
const Carousel = ({ items }: CarouselProps) => {
  ...
  const handleLeftClick = () => setItem(item === 0 ? -(items.length - 1) : item + 1);

  const handleRightClick = () => setItem(item === -(items.length - 1) ? 0 : item - 1);
  ...
  if (items.length > 0) {
    items.forEach((item, index) => {
      ...
      carouselIndicators.push(
        ...
        <div key={index} className={styles.CarouselIndicatorContainer} onClick={getIndicatorClickHandler(index)}>
